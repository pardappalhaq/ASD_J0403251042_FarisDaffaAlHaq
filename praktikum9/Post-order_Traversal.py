...
# ==========================================================
# Contoh Algoritma Traversal Pohon: Post-order Traversal
# ==========================================================
...
# Postorder traversal
# Left ->Right -> Root
    def PostorderTraversal(self, root): # Membuat method untuk melakukan traversal post-order
        res = [] # Membuat list kosong untuk menyimpan hasil traversal
        if root: # Jika node saat ini tidak kosong
            res = self.PostorderTraversal(root.left) # Panggil method PostorderTraversal pada anak kiri terlebih dahulu (traversal ke kiri)
            res = res + self.PostorderTraversal(root.right) # Panggil method PostorderTraversal pada anak kanan (traversal ke kanan) dan gabungkan hasilnya dengan list hasil traversal sebelumnya
            res.append(root.data) # Tambahkan data pada node saat ini ke dalam list hasil traversal setelah traversal ke kiri dan kanan selesai
...
print(root.PostorderTraversal(root)) # Memanggil method PostorderTraversal untuk mencetak isi tree dengan urutan post-order, yang akan menghasilkan list [10, 19, 14, 31, 42, 35, 27] karena data pada tree diurutkan sesuai aturan binary search tree dan traversal dilakukan dengan urutan left-right-root