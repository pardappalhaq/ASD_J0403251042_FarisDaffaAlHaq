...
# ==========================================================
# Contoh Algoritma Traversal Pohon: In-order Traversal
# ==========================================================
...
# Print the Tree
    def PrintTree(self): # Membuat method untuk mencetak isi tree dengan cara traversal
        if self.left: # Jika anak kiri dari node saat ini ada
...
# Inorder traversal
# Left -> Root -> Right
    def inorderTraversal(self, root): # Membuat method untuk melakukan traversal in-order
        res = [] # Membuat list kosong untuk menyimpan hasil traversal
        if root: # Jika node saat ini tidak kosong
            res = self.inorderTraversal(root.left) # Panggil method inorderTraversal pada anak kiri terlebih dahulu (traversal ke kiri)
            res.append(root.data) # Tambahkan data pada node saat ini ke dalam list hasil traversal
            res = res + self.inorderTraversal(root.right) # Panggil method inorderTraversal pada anak kanan (traversal ke kanan) dan gabungkan hasilnya dengan list hasil traversal sebelumnya
...
print(root.inorderTraversal(root)) # Memanggil method inorderTraversal untuk mencetak isi tree dengan urutan in-order, yang akan menghasilkan list [10, 14, 19, 27, 31, 35, 42] karena data pada tree diurutkan secara ascending sesuai aturan binary search tree
